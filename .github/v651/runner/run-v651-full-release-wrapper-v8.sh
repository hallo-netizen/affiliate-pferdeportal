...
TMP=/tmp/run-v651-full-release-wrapper-v8-expanded.sh
python3 - <<'PY'
...
Path('/tmp/run-v651-full-release-wrapper-v8-expanded.sh').write_text(src.replace(needle,insert,1))
PY
bash -n "$TMP"
chmod +x "$TMP"
set +e
...
set -e
if [ "$rc" -ne 0 ]; then
  D=V651_RELEASE_EVIDENCE/BLOCKED_DIAGNOSTIC_V8
