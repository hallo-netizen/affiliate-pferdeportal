...
final class PA_Affiliate_Design_Performance {
    ...
    public static function pre_setup($pre, $menu_item) {
        if ($pre !== null || !self::$active || self::$phase !== 'mobile' || !is_object($menu_item)) {
            return $pre;
        }

        $id = absint($menu_item->ID ?? 0);
        if ($id <= 0 || !isset(self::$core_setup_cache[$id])) {
            return $pre;
        }

        // Der teure Core-Setup wird wiederverwendet. Die normale Filterkette
        // läuft erneut, damit sich fachliche Erweiterungen weiterhin verhalten wie vorher.
        return apply_filters('wp_setup_nav_menu_item', clone self::$core_setup_cache[$id]);
    }

    public static function capture_setup($menu_item) {
        if (!self::$active || self::$phase !== 'desktop' || !is_object($menu_item)) {
            return $menu_item;
        }

        $id = absint($menu_item->ID ?? 0);
        if ($id > 0 && !isset(self::$core_setup_cache[$id])) {
            // Priority -999999: pristine Core-Setup vor nachgelagerten Filtern sichern.
    ...
    private static function same_location(string $location): bool {
        return self::$theme_location === $location;
    }
