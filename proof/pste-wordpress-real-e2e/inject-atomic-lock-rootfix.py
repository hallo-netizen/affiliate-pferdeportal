...
"""    private static function acquireLock(): string {$token=hash('sha256','driver|'.microtime(true).'|'.wp_generate_uuid4());$v=['token'=>$token,'expires_at'=>time()+self::LOCK_TTL];if(self::insertLockIfAbsent(PSTE_OPTION_RESEARCH_DRIVER_LOCK,$v))return $token;self::deleteExpiredLockAtomically(PSTE_OPTION_RESEARCH_DRIVER_LOCK);if(self::insertLockIfAbsent(PSTE_OPTION_RESEARCH_DRIVER_LOCK,$v))return $token;throw new RuntimeException('PSTE_RESEARCH_DRIVER_ALREADY_RUNNING');}
    private static function insertLockIfAbsent(string $option,array $value): bool {global $wpdb;$sql=$wpdb->prepare("INSERT IGNORE INTO {$wpdb->options} (option_name,option_value,autoload) VALUES (%s,%s,%s)",$option,maybe_serialize($value),'off');$r=$wpdb->query($sql);if($r===false)throw new RuntimeException('PSTE_DRIVER_LOCK_INSERT_FAILED');wp_cache_delete($option,'options');return (int)$r===1;}
    ...
    private static function deleteExpiredLockAtomically(string $option): void {global $wpdb;$raw=$wpdb->get_var($wpdb->prepare("SELECT option_value FROM {$wpdb->options} WHERE option_name=%s",$option));if(!is_string($raw)||$raw==='')return;$v=maybe_unserialize($raw);if(!is_array($v)||(int)($v['expires_at']??0)>=time())return;error_log('PSTE_LOCKTRACE DRIVER_EXPIRED_DELETE t='.sprintf('%.6f',microtime(true)).' pid='.getmypid().' token='.(string)($v['token']??'').' expires='.(string)($v['expires_at']??''));PSTE_DB_Write_Guard::query($wpdb->prepare("DELETE FROM {$wpdb->options} WHERE option_name=%s AND option_value=%s",$option,$raw),'PSTE_LOCK_ATOMIC_EXPIRED_DELETE_FAILED');wp_cache_delete($option,'options');}
    private static function deleteOwnedLockAtomically(string $option,string $token): void {global $wpdb;$raw=$wpdb->get_var($wpdb->prepare("SELECT option_value FROM {$wpdb->options} WHERE option_name=%s",$option));if(!is_string($raw)||$raw==='')return;$v=maybe_unserialize($raw);if(!is_array($v)||!hash_equals($token,(string)($v['token']??'')))return;error_log('PSTE_LOCKTRACE DRIVER_OWNED_DELETE t='.sprintf('%.6f',microtime(true)).' pid='.getmypid().' token='.$token);PSTE_DB_Write_Guard::query($wpdb->prepare("DELETE FROM {$wpdb->options} WHERE option_name=%s AND option_value=%s",$option,$raw),'PSTE_LOCK_ATOMIC_RELEASE_FAILED');wp_cache_delete($option,'options');}""","DRIVER")
