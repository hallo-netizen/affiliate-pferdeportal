...
foreach((array)($q['items']??[]) as $item){
    if((float)($item['actual_cost']??0)>0)$providerJobs++;
}
if($providerJobs<$completed)throw new RuntimeException('PROVIDER_JOB_COUNT_LT_COMPLETED_'.$providerJobs.'_'.$completed);
foreach(['/v3/dataforseo_labs/google/keyword_suggestions/live','/v3/dataforseo_labs/google/related_keywords/live','/v3/dataforseo_labs/google/keyword_ideas/live','/v3/serp/google/organic/task_post'] as $p){
    if((int)($counts[$p]??0)!==$providerJobs)throw new RuntimeException('PROVIDER_COUNT_MISMATCH_'.$p.'_'.(int)($counts[$p]??0).'_'.$providerJobs);
}
$get=0;foreach($counts as $p=>$n)if(str_contains((string)$p,'/task_get/advanced/'))$get+=(int)$n;
if($get!==$providerJobs)throw new RuntimeException('PAA_GET_COUNT_MISMATCH_'.$get.'_'.$providerJobs);
echo wp_json_encode(['status'=>'PASS_REAL_WORDPRESS_BREADTH','completion_reason'=>$q['completion_reason'],'families'=>$completed,'provider_jobs'=>$providerJobs,'usable'=>$usable,'provider_counts'=>$counts],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)."\n";
