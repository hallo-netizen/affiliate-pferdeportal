# BAUCONTAINER – BAUÄNDERUNGEN

STAND: 2026-09-05

Dieses Dokument ist nur der Architektur-Index.
Die vollständige Begründung liegt im zentralen `AENDERUNGSREGISTER.md`.

- ARCH-001 → Campus/Bürogebäude-Modell
- ARCH-002 → Hauptpförtner als Resetpunkt
- ARCH-003 → ein Hobbyraum pro Büro
- ARCH-004 → Paul isoliert
- ARCH-005 → modularer Campus
- ARCH-006 → PROJECT_MEMORY unter bestehender Hardlock-Abdeckung
- ARCH-007 → Notfall-Tresor
- ARCH-008 → organischer Campus
- ARCH-009 → ALLGEMEINGÜLTIGE BAUSTEINE
- ARCH-010 → vollständige Masterdatei-Verwertung
- ARCH-011 → BILD-Büro
- ARCH-012 → zentrales Modulregister
- ARCH-013 → Modulklasse und Masterakte getrennt
- ARCH-014 → Oberwachtmeister als Ablauf
- ARCH-015 → Zielvertragsraum
- ARCH-016 → Campus-Archiv
- ARCH-017 → Zugriffsschutzentscheidung
- ARCH-018 → Architektur-Sonderrecht für jeden Chat
- ARCH-019 → Bauprotokoll + Architektur-Fehlerkiste
- ARCH-020 → Campus-Hausmeister
- ARCH-021 → Pförtner/Hausmeister reine Verwaltung
- ARCH-022 → 1-Klick-Eingangsstandard

Keine Duplikation der ausführlichen Begründungen hier.
