...
bash "$R/test-fresh-0.2.10-rc6.sh"

# Real Pferde category recognition uses the actual WordPress page hierarchy.
...
echo UGE0210RC6_REAL_DESIGN_CATEGORY_HIERARCHY_PASS

# Existing historical test term still has the portal-category contract used by
# the old regression. New cluster terms use the Glossar taxonomy category link.
...
# Delete all persisted Glossar rewrites with the current schema already stored.
...
# Reproduce the live white-page failure: empty the normal main loop after query
# parsing. Direct Glossar URL truth must still produce the complete single article.
...
for slug in hufbein hufrehe strahlfaeule hufabszess; do
  ...
  grep -q 'id="uge-pftk-breadcrumb-payload"' /tmp/rd-$slug
  ! grep -q 'class="uge-breadcrumbs"' /tmp/rd-$slug
  ! grep -qiE 'fatal error|critical error' /tmp/rd-$slug
done
grep -q FULL-HUFBEIN-SENTINEL /tmp/rd-hufbein
grep -q 'class="uge-primary-category"' /tmp/rd-hufbein
for slug in hufrehe strahlfaeule hufabszess; do
  grep -q 'class="uge-related-links"' /tmp/rd-$slug
  grep -q 'class="uge-glossary-category-link"' /tmp/rd-$slug
done
...
echo UGE0210RC6_REAL_DESIGN_CLICKABILITY_BREADCRUMB_LOOP_POISON_PASS
