...
sed -e 's/0\.2\.6/0.2.8-rc1/g' -e 's/UGE026/UGE028RC1/g' -e 's/uge026/uge028rc1/g' "$SRC" > /tmp/boot-rc1.sh
chmod +x /tmp/boot-rc1.sh
! grep -q '0\.2\.6' /tmp/boot-rc1.sh
bash /tmp/boot-rc1.sh
bash "$R/exact-0.2.5-test/03_seed.sh"
bash "$R/exact-0.2.5-test/04_frontend.sh"
bash "$R/exact-0.2.5-test/05_regression.sh"
test "$(docker exec wp wp plugin list --name=universal-glossary-engine --field=version --allow-root)" = 0.2.8-rc1
...
echo UGE028RC1_FRESH_FULL_POSITIVE_NEGATIVE_PASS
