...
- `Pferd` oder `Pferderasse` muss genau einmal in der H1 vorkommen;
- zweite Hälfte nur aus belegter Herkunft, Region, Typ, Nutzung oder besonderem Merkmal;
- keine Werbung, Wertung oder Superlative;
- keine freien Adjektive wie faszinierend, edel, außergewöhnlich, perfekt, vielseitig, beliebt;
- möglichst 8–10 Wörter, nur bei sprachlicher Notwendigkeit länger.

## 3. PFERD-/PFERDERASSE-REGEL
- H1: genau einmal `Pferd` oder `Pferderasse`;
- Fließtext: mindestens einmal `Pferderasse` innerhalb der ersten 150 Wörter;
...
Keine zusätzliche H2. Keine H2 darf entfallen.

## 5. ABSCHNITTSBINDUNG
Jedes Datenfeld besitzt einen fachlich passenden Hauptabschnitt. Fakten dürfen nicht beliebig verschoben werden.

Beispiele:
- Herkunft/Entstehung/ursprüngliche Nutzung → Herkunft und Geschichte bzw. Ursprüngliche Nutzung;
...
- Status
- Stockmaß
- Gewicht nur wenn belegt; sonst `nicht belastbar dokumentiert`
- Typische Farben
- Besondere Gangarten
- Ursprüngliche Nutzung
...
## 10. ZUSAMMENFASSUNG
- letzte H2 lautet immer exakt `## Zusammenfassung`;
- ca. 70–120 Wörter;
