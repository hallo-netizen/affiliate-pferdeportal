...
final class APKW_DeltaPlanner {
    ...
    public static function plan(array $baseline,array $candidate):array{
        ...
        foreach($next as $id=>$node){
            ...
            $old=$base[$id];$fields=[];
            foreach(['name','slug','parent_concept_id','level','primary_keyword','research_cluster_id','search_intent','status','affiliate_fit','affiliate_path','affiliate_fit_justification'] as $f){if(self::norm($old[$f]??null)!==self::norm($node[$f]??null))$fields[]=$f;}
