#!/usr/bin/env python3
"""EXAKTE HISTORISCHE KOPIE vom Commit a4084afd2b22cdbab2e7c6141885760adedc8f4b."""
...
def validate(env):
    ...
    for i,item in enumerate(plan.get('items',[])):
        ...
        if not fp: errors.append({'code':'FACT_PACK_FOR_PLAN_ITEM_NOT_FOUND','index':i,'source_snapshot_id':sid}); continue
        title_scope=norm(fp.get('title_scope')); subject_scope=norm((item.get('runtime_order') or {}).get('subject_scope'))
        if not title_scope or title_scope != subject_scope: errors.append({'code':'CANONICAL_FACT_PACK_SCOPE_MISMATCH_PREVENTED','index':i,'fact_pack_id':sid,'title_scope':title_scope,'subject_scope':subject_scope})
    for i,item in enumerate(plan.get('items',[])):
        ...
        if not name or not slug or taxonomy != 'category' or not path: errors.append({'code':'WORDPRESS_CATEGORY_BINDING_INCOMPLETE','index':i,'name':name,'slug':slug,'taxonomy':taxonomy,'hierarchy_path':path})
        if leaf and name != leaf: errors.append({'code':'WORDPRESS_CATEGORY_NAME_NOT_HIERARCHY_LEAF','index':i,'name':name,'expected_name':leaf,'slug':slug})
        if name == slug and leaf and leaf != slug: errors.append({'code':'WORDPRESS_CATEGORY_NAME_WAS_SYNTHESIZED_FROM_SLUG','index':i,'name':name,'slug':slug,'expected_name':leaf})
    ...
    for field,objkey in [('fact_pack_bundle_sha256','fact_pack_bundle'),('production_plan_sha256','production_plan'),('workflow_release_sha256','workflow_release')]:
        ...
        if expected and expected != stable_hash(env.get(objkey)): errors.append({'code':'PACKAGE_COMPONENT_HASH_MISMATCH','field':field,'expected':expected,'actual':stable_hash(env.get(objkey))})
