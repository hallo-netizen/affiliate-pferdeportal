...
def stage_payloads(index: int):
    ...
    evidence = f'Konkrete Forschungsquelle für Artikel {index}; dieser Text ist lang genug für den strukturierten Research-Nachweis.'
    research = {
    ...
    fact_pack = {
        ...
        'sources': [{key: research['sources'][0][key] for key in ('source_id','source_title','source_url','retrieved_at','snapshot_sha256')}],
        'claims': claims,
    }
...
class RepairContinuityTests(unittest.TestCase):
    ...
    def test_real_tool_failure_remains_hard_fail_closed(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); workspace = root / 'item0'
            prepare_workspace(root, workspace, 0)
            controller.cmd_draft(workspace, write_stage_file(root, 'draft.html', draft_for(0)))
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            def fake_run_all(repo, state, fact_pack, production_plan_item):
                ...
                if state['article']['title'] == 'Mistcontainer mit Deckel wählen' and count == 1:
                    raise controller.production_checks.RepairRequired('languagetool',[{'error_code':'LANGUAGETOOL_FINDING','rule_id':'GERMAN_SPELLER_RULE'}])
            ...
            self.assertEqual(revisions, [1,1,2,1,1,1,1]); self.assertEqual(sum(per_slot_calls.values()), 8)
            out = root / 'batch'; result = batch_gate.collect_batch(snapshot, state_paths, out)
            self.assertEqual(result['status'], 'SYSTEM4_BATCH_FULL_PASS_COLLECTED'); self.assertEqual(result['article_count'], 7)
            self.assertEqual(result['batch_sha256'], '7f2e3290b6ac78ac7df1644395e57ac72f02dc1373e390eb2e532e57a8ce916a'); self.assertFalse(result['publish_allowed'])
