...
PPM_HARD_RULES = [
    ...
    # W4::R8-0790 is a process/evidence requirement (LOCAL_EXECUTED_TEST), not writer content.
...
class TextmachineHardBlockMatrixTests(unittest.TestCase):
    ...
    def _controller_with_content_error(self, code):
        state = self.state()
        path = Path('/tmp/system4-hardblock-content.json')
        with mock.patch.object(controller, 'load', return_value=(state, path)), \
             ...
             mock.patch.object(controller.content_guard, 'validate_single_article', side_effect=content_guard.ContentGuardError(code)), \
    ...
    def _controller_with_design_error(self, code):
        state = self.state()
        path = Path('/tmp/system4-hardblock-design.json')
        with mock.patch.object(controller, 'load', return_value=(state, path)), \
             mock.patch.object(controller.authoring_contract, 'validate_bound', return_value={}), \
             mock.patch.object(controller.content_guard, 'validate_single_article', return_value={'status': 'PASS'}), \
             mock.patch.object(controller.design_guard, 'validate_design_neutrality', side_effect=design_guard.DesignGuardError(code)), \
             mock.patch.object(controller._engine, 'save', return_value=None):
    ...
    def _controller_with_production_hard_error(self, code):
        state = self.state()
        path = Path('/tmp/system4-hardblock-ppm.json')
        with mock.patch.object(controller, 'load', return_value=(state, path)), \
             mock.patch.object(controller.authoring_contract, 'validate_bound', return_value={}), \
             mock.patch.object(controller.content_guard, 'validate_single_article', return_value={'status': 'PASS'}), \
             mock.patch.object(controller.design_guard, 'validate_design_neutrality', return_value={'status': 'PASS'}), \
             mock.patch.object(controller.production_checks, 'run_all', side_effect=production_checks.ProductionCheckError(code)), \
             mock.patch.object(controller._engine, 'save', return_value=None):
    ...
    def test_all_28_content_guard_hard_rules_never_become_repair(self):
        ...
        for code in CONTENT_GUARD_HARD:
            with self.subTest(code=code):
                self.assertIsNone(production_checks.guard_repair_finding('content_guard', code))
                self._controller_with_content_error(code)

    def test_all_7_design_guard_hard_rules_never_become_repair(self):
        self.assertEqual(len(DESIGN_GUARD_HARD), 7)
        for code in DESIGN_GUARD_HARD:
            with self.subTest(code=code):
                self.assertIsNone(production_checks.guard_repair_finding('design_guard', code))
                self._controller_with_design_error(code)

    def test_matrix_control_sum_is_exactly_50(self):
        self.assertEqual(len(PPM_HARD_RULES) + len(CONTENT_GUARD_HARD) + len(DESIGN_GUARD_HARD), 50)
