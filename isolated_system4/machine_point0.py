...
def build_from_acquired(*, snapshot_bytes:bytes, acquired_batch:dict, prewrite_plan_batch:dict, provider:str, manifest:str, head:str)->dict:
    prod=point0_snapshot._validate_prod(snapshot_bytes); items=prod['next_textmachine_metadata_batch']['items']
...
def main(argv:list[str])->int:
    try:
        ...
        if argv[1]=='build-fetch': source_value=source_acquisition.acquire_batch(source_value)
