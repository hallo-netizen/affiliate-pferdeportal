...
def handoff_from_state(f,state_path,out):
    ...
    payload={'contract':handoff_transport.HANDOFF_CONTRACT,'batch_sha256':s['batch_sha256'],'publish_allowed':False,'signing_deferred':True,'batch_gate_status':'SYSTEM4_BATCH_FULL_PASS_COLLECTED','no_legacy_status':'PASS','test_suite_status':'PASS','wordpress_review':handoff_transport.wordpress_review(),'articles':[{'index':0,'title':a['title'],'target_keyword':a['target_keyword'],'category':a['category'],'article_type':a['article_type'],'plan_slot':a['plan_slot'],'final_draft_sha256':s['draft_sha256'],'revision_count':s['revision'],'body':s['draft_markdown'],'production_context':{'fact_pack':s['production_context']['fact_pack'],'production_plan_item':s['production_context']['production_plan_item']},'languagetool':prod['languagetool'],'ppm679':prod['ppm679']}]}
