...
- `design_guard.py` is validation-only. It reproduces the already-documented existing production/design boundary (`ppm-generated`, exact article-type class/attribute, `system-129-table comparison-table`, no inline/global styling payload, Beratung section headings on the existing H2 level). It performs ZERO mutation. A mismatch is BLOCKED before WordPress.
...
- A batch must additionally pass the System-4 cross-article distinctness check. The known 7er failure class — one reusable sentence/paragraph template across different topics — is BLOCKED before collection/handoff. This is a release-integrity check across finished articles, not a Textmaschine rewrite rule.
...
- The final handoff validator rechecks the bound fact packs/fact traces, unchanged-design boundary and cross-article distinctness; a previously seen source-free, design-drifted or template-reuse batch cannot become a valid Chat/WordPress file merely because transport hashes are correct.
