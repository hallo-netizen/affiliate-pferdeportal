...
## Current whole path
WordPress JSON metadata batch -> same indexed ingress for item 0..N-1 -> ONE persistent canonical state per article -> research -> fact check -> draft -> FULL production checks -> repair of the SAME draft if FAIL -> output gate -> 7-item batch gate -> one real batch file to Chat.
...
## Codex economy
Codex is reserved for a REAL bound article-production run only. Diagnostics, architecture, preflight repair, test-only proof, GitHub persistence experiments, signing and WordPress work must not consume Codex when they can be done otherwise. Before a future Codex start, entry/preflight and a supported real-file/artifact return path must already be confirmed. Seven full article bodies must not be returned as a giant PR comment.
...
- Proof-persistence guard: test-only hardening; local corrected suite 4/4 PASS. It is not a requirement to store production article bodies in GitHub.
- Signature bridge: isolated local positive/negative test exists, but signing/WordPress are deferred and no production/live WordPress PASS is claimed.

## Additional local evidence not promoted to durable branch PASS
- Article 2 (`Eine richtige Reitplatzbeleuchtung ohne Mast finden`): local real LT 6.8 final 0 findings and real PPM 6.7.9 Technical/Content/Aggregate PASS; no durable branch production proof was materialized.
- Article 3: local LT 6.8 final 0 findings only; final PPM FULL PASS not completed/proven.
...
## NEXT ACTION
1. Do not start Codex yet.
2. Confirm a real retrievable one-file Codex/task handoff without spending Codex quota.
3. Fresh-read the System-4 branch head and run the existing local preflight/no-legacy checks without Codex.
