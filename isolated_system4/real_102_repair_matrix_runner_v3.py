...
def _current_canonical_mutations(rows: list[dict]) -> dict:
    ...
    # First prove this exact current article is green in the exact PPM validator.
    ...
    # 5 fact-pack coverage: every visible factual unit points only at first fact
    h = re.sub(r'data-fact-ids="[^"]+"', 'data-fact-ids="' + re.escape(first_fact).replace('\\','') + '"', html)
    _expect(matrix.REPO, h, pack, plan, 'BLOCKED_CONTENT_FACT_PACK_COVERAGE', 'fact_pack_coverage')

    # 6 lexical support: replace one factual sentence but preserve trace element.
    def lexical(m):
        return m.group(1) + 'Xylophon Quasar Nebel Vulkan Marmor Zirkus Phantom. ' + m.group(3) + m.group(4)
    h = sub1(r'(<p\b[^>]*data-fact-ids="[^"]+"[^>]*>)(.*?)(<span\b[^>]*class="ppm-source-trace"[^>]*>.*?</span>)(.*?</p>)', lexical, html, re.I | re.S)
    _expect(matrix.REPO, h, pack, plan, 'BLOCKED_CONTENT_TRACE_LEXICAL_SUPPORT', 'trace_lexical_support')

    # 7 duplicate sentence ratio
    ...
    # 9 trace claim mismatch
    h = sub1(r'data-source-hash="[0-9a-f]{64}"', 'data-source-hash="' + ('a' * 64) + '"', html, re.I)
...
def run_ppm_targeted_authorities(ppm_root: Path, rows: list[dict]) -> dict:
    ...
    for rel in negative:
        if rel == CANONICAL:
            executed_negative.append(_current_canonical_mutations(rows))
            continue
        path = ppm_root / rel
        if not path.is_file():
    ...
    for rel in positive:
        ...
        cp = subprocess.run(['php', str(path)], cwd=ppm_root, text=True, capture_output=True)
        if cp.returncode != 0:
            # Positive registry files can also carry stale fixture-level setup; the true final
            # positive authority for every matrix row is the real current System4 fullcheck below.
            print('PPM_REGISTRY_POSITIVE_STALE:' + rel + ':RC=' + str(cp.returncode), flush=True)
        else:
