...
def verify_bundle(bundle:dict, *, actual_manifest:str, actual_head:str)->tuple[dict,bytes]:
    ...
    if wc.get('contract')!='SYSTEM4_BOUND_WORKER_DISPATCH_V2' or wc.get('external_web_search_allowed') is not False or wc.get('publish_allowed') is not False or wc.get('machine_prewrite_mutation_allowed') is not False: raise WorkerDispatchError('DISPATCH_WORKER_CONTRACT_INVALID')
