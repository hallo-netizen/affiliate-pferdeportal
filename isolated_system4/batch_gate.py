...
NEXT_REQUIRED = 'PARENT_CHAT_WORDPRESS_HANDOFF_REQUIRED'
...
def validate_full_production_evidence(checks: Mapping[str, Any], draft_sha: str) -> dict[str, Any]:
    ...
    if not isinstance(prod, dict):
        raise BatchGateError('FULL_PRODUCTION_EVIDENCE_MISSING')
    if prod.get('contract') != 'SYSTEM4_FULL_PRODUCTION_CHECK_V1' or prod.get('status') != 'PASS':
        raise BatchGateError('FULL_PRODUCTION_EVIDENCE_INVALID')
    if prod.get('checked_draft_sha256') != draft_sha:
        raise BatchGateError('FULL_PRODUCTION_EVIDENCE_DRAFT_MISMATCH')
    if prod.get('publish_allowed') is not False:
        raise BatchGateError('FULL_PRODUCTION_EVIDENCE_PUBLISH_MUST_BE_FALSE')
    evidence = prod.get('evidence')
    if not isinstance(evidence, dict):
        raise BatchGateError('FULL_PRODUCTION_NESTED_EVIDENCE_MISSING')
    no_legacy = evidence.get('no_legacy')
    no_links = evidence.get('no_external_links')
    lt = evidence.get('languagetool')
    ppm = evidence.get('ppm679')
    if not isinstance(no_legacy, dict) or no_legacy.get('status') != 'PASS' or no_legacy.get('legacy_import_count') != 0:
        raise BatchGateError('NO_LEGACY_EVIDENCE_NOT_PASS')
    if not isinstance(no_links, dict) or no_links.get('status') != 'PASS' or no_links.get('external_link_count') != 0:
        raise BatchGateError('EXTERNAL_LINK_EVIDENCE_NOT_PASS')
    if not isinstance(lt, dict) or lt.get('status') != 'PASS' or lt.get('engine') != 'LanguageTool 6.8 / Bestand 43' or lt.get('finding_count') != 0:
        raise BatchGateError('LANGUAGETOOL_EVIDENCE_NOT_PASS')
    if not isinstance(ppm, dict) or ppm.get('status') != 'PASS' or ppm.get('ppm_version') != '6.7.9':
        raise BatchGateError('PPM679_EVIDENCE_NOT_PASS')
    if ppm.get('technical_status') != 'TECHNICAL_CHECK_OK' or ppm.get('content_quality_status') != 'CONTENT_QUALITY_CHECK_OK' or ppm.get('fail_closed_aggregate_status') != 'PASS':
        raise BatchGateError('PPM679_EVIDENCE_NOT_PASS')
    if ppm.get('content_sha256') != draft_sha:
        raise BatchGateError('PPM679_CONTENT_HASH_MISMATCH')
    source = ppm.get('language_evidence_source')
    if source not in ('BOUND_CURRENT_REUSED', 'REAL_LT68_FULLCHECK_REUSED', 'REAL_LT68_CURRENT_DRAFT_REFRESHED'):
        raise BatchGateError('PPM679_LANGUAGE_EVIDENCE_SOURCE_INVALID')
    return {
        'languagetool': {'engine': lt['engine'], 'status': 'PASS', 'finding_count': 0},
        'ppm679': {
            'version': '6.7.9',
            'status': 'PASS',
            'technical_status': ppm['technical_status'],
            'content_quality_status': ppm['content_quality_status'],
            'fail_closed_aggregate_status': ppm['fail_closed_aggregate_status'],
            'language_evidence_source': source,
        },
        'no_legacy': 'PASS',
        'external_links': 'PASS',
    }
...
def validate_state(state, expected_article, source_snapshot_sha256, batch_sha256):
    ...
    try:
        ...
        raise BatchGateError('CONTENT_CONTEXT_NOT_PASS:' + str(exc)) from exc
    try:
        design_summary = design_guard.validate_design_neutrality(draft, expected_article['article_type'])
    except design_guard.DesignGuardError as exc:
        raise BatchGateError('DESIGN_GUARD_NOT_PASS:' + str(exc)) from exc
    return {
...
def collect_batch(snapshot_path: Path, state_paths: Sequence[Path], out_dir: Path):
    ...
    for item in items:
        ...
        article_rows.append({
            ...
            'design': validated['design_summary'],
        })
        state_rows.append({
            'plan_slot': item['plan_slot'],
    ...
    evidence = {
        ...
        'batch_distinctness': diversity,
        'batch_repetition': repetition,
