...
CONTRACT='SYSTEM4A_CHAT_TO_FILE_FULL_ACCEPTANCE_V3'
...
def git_head()->str:
 ...
 req(cp.returncode==0,'RUNTIME_HEAD_UNAVAILABLE')
 return cp.stdout.strip()
...
def verify_proof_manifest()->tuple[dict[str,Any],dict[str,str]]:
 path=SYSTEM4/'CURRENT_PROVEN_RUNTIME_MANIFEST.json'
 req(path.is_file(),'CURRENT_PROVEN_RUNTIME_MANIFEST_MISSING')
 m=json.loads(path.read_text(encoding='utf-8'))
 req(m.get('contract')==MANIFEST_CONTRACT,'CURRENT_PROVEN_RUNTIME_MANIFEST_INVALID')
 proof_head=m.get('proof_head'); req(isinstance(proof_head,str) and len(proof_head)==40,'PROOF_HEAD_INVALID')
 req(git_head()==proof_head,'CURRENT_HEAD_NOT_PROVEN_HEAD')
 blobs=m.get('git_blobs'); req(isinstance(blobs,dict),'RUNTIME_BLOB_MANIFEST_MISSING')
 actual={}
 ...
 total=hist.get('total'); passed=hist.get('passed'); failed=hist.get('failed')
 req(isinstance(total,int) and total>=len(LIVE_ESCAPE_CLASSES),'HISTORICAL_MATRIX_INCOMPLETE')
 req(passed==total and failed==0,'HISTORICAL_MATRIX_NOT_FULL_PASS')
 ...
 req(zero.get('real_languagetool_6_8') is True,'ZERO_TO_FILE_REAL_LT_MISSING')
 req(zero.get('real_ppm_6_7_9') is True,'ZERO_TO_FILE_REAL_PPM_MISSING')
 ...
 req(m.get('simulation_and_live_same_entry') is True,'SIMULATION_LIVE_ENTRY_PARITY_NOT_PROVEN')
 return m,actual
...
def verify_terminal(r:dict[str,Any],count:int)->None:
 ...
 req(r.get('article_count')==count,'FINAL_ARTICLE_COUNT_MISMATCH'); req(r.get('codex_used') is False,'CODEX_USED_IN_SIMULATION')
 ...
 for i,b in enumerate(bindings):
  ...
  for k in ('title','target_keyword','category','plan_slot','links','textmachine_contract_sha256'): req(b.get(k),f'MACHINE_BINDING_FIELD_MISSING:{i}:{k}')
...
def run(trigger:Path,work:Path,lt:Path)->dict[str,Any]:
 chat=load_raw_chat_trigger(trigger); manifest,identity=verify_proof_manifest()
 ...
 req(ppm.is_file(),'PPM679_PACKAGE_MISSING'); req(sha256_file(ppm)=='acbda93bd1c4292de7aaf88db2195631103991ff508b36c88cb694714818abd1','PPM679_PACKAGE_SHA_MISMATCH')
 result=run_live_entry(manifest,trigger,work,lt); verify_terminal(result,chat['article_count'])
 return {'status':'SYSTEM4A_CHAT_TO_FILE_FULL_ACCEPTANCE_PASS','article_count':chat['article_count'],'codex_used':False,
         'historical_escape_classes_bound':len(LIVE_ESCAPE_CLASSES),'runtime_identity':identity,'publish_allowed':False,'live_result':result}
