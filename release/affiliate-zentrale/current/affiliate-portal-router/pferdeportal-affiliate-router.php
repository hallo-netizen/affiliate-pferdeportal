...
JS;
    ...
    private function assignment_selection_for_slot($context, $slot_type) {
        ...
        $repair_reason = sanitize_text_field((string) ($data['repair_reason'] ?? ''));
        if ($repair_reason !== '') {
            $suffix .= ' · Reparaturgrund: ' . $repair_reason;
        }
...
JS;
    ...
    public function handle_save_banner_distribution() {
        ...
        update_option(self::OPTION_BANNER_DISTRIBUTION, array(
            'enabled'=>$enabled,
            'mode'=>'relevance_first_dynamic_coverage',
            'updated_at'=>time(),
    ...
    public function handle_save_assignment() {
        ...
        $repair_reason = sanitize_text_field((string) ($raw['repair_reason'] ?? ''));
        $manual_repair = $banner_mode !== 'automatic' || $products_mode !== 'automatic';
        if ($manual_repair && $repair_reason === '') {
            wp_die('Für eine manuelle Affiliate-Reparatur ist eine Begründung erforderlich.');
        }
        $all=$this->get_assignments(); $all[$page_id]=array(
            'banner_mode'=>$banner_mode,
            'banner_id'=>absint($raw['banner_id']??0),
            'products_mode'=>$products_mode,
            'product_ids'=>$product_ids,
            'repair_reason'=>$manual_repair ? $repair_reason : '',
            'apply_descendants'=>!empty($raw['apply_descendants']),
            'updated'=>time(),
            'updated_by'=>get_current_user_id(),
    ...
    public function render_assignments_page() {
        ...
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="max-width:900px;background:#fff;border:1px solid #c3c4c7;padding:18px;margin-bottom:24px">
            ...
            <p><label><input type="checkbox" name="ppar_banner_distribution[enabled]" value="1" <?php checked(!empty($banner_distribution['enabled'])); ?>> <strong>Dynamische Bannerverteilung aktiv</strong></label></p>
            <p class="description"><strong>Relevanz zuerst.</strong> Nur innerhalb derselben besten Relevanzstufe wird verteilt: zuerst an den bisher am wenigsten abgedeckten Partner, danach an dessen bisher am wenigsten abgedecktes Werbemittel. Feste Providerquoten und Wochenrotation werden nicht verwendet.</p>
            <?php submit_button('Verteilung speichern'); ?>
        </form>
