        ...
        <div class="wrap" style="max-width:1180px">
            ...
            <?php if ($provider === 'awin') : $awin_settings=$this->network_settings('awin'); $awin_programmes=get_option(self::OPTION_NETWORK_AWIN_PROGRAMMES,array()); $awin_programmes=is_array($awin_programmes)?$awin_programmes:array(); ?>
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:16px">
                    <section class="postbox" style="padding:18px"><h2>Programme &amp; Partner</h2><p>Advertiser aufnehmen, Geschäftsbereich bestätigen und Portal-Freigabe steuern.</p><p><strong>Aktuell erkannte verbundene Programme:</strong> <?php echo absint(count($awin_programmes)); ?></p><?php $awin_refresh_state=sanitize_key((string)($_GET['ppar_awin_refresh']??'')); if($awin_refresh_state==='ok'): ?><div class="notice notice-success inline"><p>Awin-Programmliste aktualisiert: <?php echo absint($_GET['ppar_awin_refresh_count']??count($awin_programmes)); ?> verbundene Programme.</p></div><?php elseif(in_array($awin_refresh_state,array('failed','not_ready'),true)): ?><div class="notice notice-error inline"><p><?php echo esc_html((string)($_GET['ppar_awin_refresh_message']??'Awin-Programmliste konnte nicht aktualisiert werden.')); ?></p></div><?php endif; ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="display:inline-block;margin-right:8px"><input type="hidden" name="action" value="ppar_refresh_awin_programmes"><?php wp_nonce_field('ppar_refresh_awin_programmes','ppar_awin_refresh_nonce'); ?><button class="button button-primary" type="submit">Programme jetzt aktualisieren</button></form><a class="button" href="<?php echo esc_url(admin_url('admin.php?page=affiliate-portal-partner-intake&provider=awin')); ?>">Awin-Partner öffnen</a></section>
                ...
                <section class="postbox" style="padding:18px;margin-top:18px;max-width:900px"><h2>Produktfeed-Betriebsprofil</h2><p>Diese Zuordnung ist Providerlogik und gehört deshalb nicht zu den zentralen Zugangsdaten.</p>
                    ...
                    <p><label><input type="checkbox" name="ppar_network[awin][product_feed_scope]" value="portal_filtered" <?php checked((string)($awin_settings['product_feed_scope']??''),'portal_filtered'); ?>> Dieser Export wurde in Awin <strong>Create-a-Feed</strong> auf für das Pferde Atelier relevante Kategorien eingegrenzt.</label></p>
