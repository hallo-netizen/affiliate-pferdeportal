...
trait PPAR_Control_Contract_Trait {
    ...
    public function control_creative_safety_check($row, $portal, $stage = 'plan') {
        ...
        // Verified eBay BUSINESS product cards have a separate remote-image
        // contract: the provider image URL plus concept_v3 proof is sufficient
        // for contain-rendering while dimension measurement remains a later
        // quality/health signal. output_format_slot() already carries this
        // contract; the central safety gate must not contradict it and strand
        // every newly discovered product at 0x0 before it can materialize.
        $provisional_ebay_product = method_exists($this, 'output_allows_provisional_ebay_product_asset')
            && $this->output_allows_provisional_ebay_product_asset($row);
        if ((absint($row['width'] ?? 0) <= 0 || absint($row['height'] ?? 0) <= 0) && !$provisional_ebay_product) {
            return new WP_Error('control_dimensions_missing', 'Reale Bildmaße fehlen.');
        }
        $payload = json_decode((string) ($row['payload'] ?? ''), true);
        $payload = is_array($payload) ? $payload : array();
        if (!in_array((string) ($payload['_dimension_state'] ?? ''), array('verified','mismatch'), true) && !$provisional_ebay_product) {
