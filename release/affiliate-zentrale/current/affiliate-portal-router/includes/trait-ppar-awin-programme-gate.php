...
trait PPAR_Awin_Programme_Gate_Trait {
    ...
    public function run_awin_programme_inventory_refresh() {
        ...
        if (sanitize_key((string) ($result['status'] ?? '')) === 'refreshed') {
            $settings = $this->network_settings('awin');
            $settings['programme_count'] = absint($result['count'] ?? 0);
            $settings['last_checked'] = time();
            $settings['last_status'] = 'connected';
            $settings['last_message'] = 'Awin-Programmliste automatisch aktualisiert: ' . absint($result['count'] ?? 0) . ' verbundene Programme.';
