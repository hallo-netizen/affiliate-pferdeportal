...
def make_full_package(path:Path):
    ...
    release={'contract':'WORKFLOW_SUPERVISOR_RELEASE_V2_SIGNED','status':'PASS','signature_algorithm':'ED25519','signing_key_id':key_id,'signing_public_key_sha256':key_sha,'fact_pack_bundle_sha256':bh,'production_plan_sha256':ph,'exact_five_batch_sha256':state['batch_sha256'],'exact_five_item_count':len(slots),'items':release_items}
    rph=stable(release); release['release_payload_sha256']=rph; release['signature_b64']=base64.b64encode(private.sign(rph.encode('ascii'))).decode(); release['release_sha256']=stable(release); rh=stable(release)
...
def run():
    ...
    with tempfile.TemporaryDirectory() as td:
        td=Path(td); pkg=td/'full.json'; env,trusted=make_full_package(pkg); planp=td/'plan.json'; make_recovery_plan(planp)
        ...
        expect_block(lambda: gate.validate_package(planp,REPO,True,trusted),'INTERMEDIATE_PRODUCTION_PLAN_NOT_UPLOADABLE'); ok('NEGATIVE_PRODUCTION_PLAN_BLOCKED')
