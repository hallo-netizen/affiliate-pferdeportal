...
REPO = HERE.parents[1]
MANIFEST = HERE / "BOUND_START_HARDLOCK.json"
RECEIPT = HERE / "BOUND_START_RECEIPT.json"
STATE = HERE / "CURRENT_STATE.json"
ROOT = HERE / "PFERDE_ATELIER_START_HERE.json"
TEXT_START_AUTH = REPO / ".text-start-receiver-auth.json"

CONTRACT = "PFERDE_ATELIER_BOUND_START_HARDLOCK_V1"
RECEIPT_CONTRACT = "PFERDE_ATELIER_BOUND_START_RECEIPT_V1"
...
def _load(path: Path) -> dict:
    try:
        ...
        raise BoundStartBlocked("BOUND_START_JSON_INVALID:" + path.name) from exc
    if not isinstance(value, dict):
        raise BoundStartBlocked("BOUND_START_OBJECT_REQUIRED:" + path.name)
    return value
...
def _git_head(repo: Path) -> str:
    try:
        ...
        raise BoundStartBlocked("BOUND_START_GIT_HEAD_UNAVAILABLE") from exc
    value = cp.stdout.strip()
    if len(value) != 40:
        raise BoundStartBlocked("BOUND_START_GIT_HEAD_INVALID")
    return value
...
def validate_payload(
    *,
    ...
    if manifest.get("contract") != CONTRACT or manifest.get("status") != "ACTIVE_FAIL_CLOSED":
        raise BoundStartBlocked("BOUND_START_MANIFEST_INVALID")
    guard_ids = {
        str(row.get("id") or "")
    ...
    if missing:
        raise BoundStartBlocked("BOUND_START_HISTORICAL_GUARD_MISSING:" + ",".join(missing))
    if manifest.get("canonical_start_command") != "python3 isolated_system4/parent_start.py start-current-bound":
        raise BoundStartBlocked("BOUND_START_CANONICAL_START_DRIFT")
    if manifest.get("canonical_step_id") != "RUN_NEW_ARTICLE_BATCH_NO_STOP" or int(manifest.get("canonical_sequence", -1)) != 107007:
        raise BoundStartBlocked("BOUND_START_CANONICAL_STEP_DRIFT")
    immutable = manifest.get("immutable_expectations") or {}
    if immutable.get("article_bodies") != "NEW":
        raise BoundStartBlocked("BOUND_START_NEW_ARTICLE_POLICY_INVALID")
    if immutable.get("article_advance") != "ONLY_AFTER_REAL_PASS":
        raise BoundStartBlocked("BOUND_START_ADVANCE_POLICY_INVALID")
    if immutable.get("repair") != "SAME_ARTICLE_SAME_WORKSPACE_UNTIL_PASS":
        raise BoundStartBlocked("BOUND_START_REPAIR_POLICY_INVALID")
    if immutable.get("quality_threshold_changes_allowed") is not False:
        raise BoundStartBlocked("BOUND_START_QUALITY_CHANGE_POLICY_INVALID")
    if immutable.get("worker_side_github_write_required") is not False:
        raise BoundStartBlocked("BOUND_START_WORKER_GITHUB_POLICY_INVALID")
    if immutable.get("publish_allowed") is not False:
        raise BoundStartBlocked("BOUND_START_PUBLISH_POLICY_INVALID")

    if root.get("current_state_sha256") != state_sha256:
        raise BoundStartBlocked("BOUND_START_STATE_HASH_BINDING_MISMATCH")
    if root.get("next_allowed_step") != state.get("next_allowed_step"):
        raise BoundStartBlocked("BOUND_START_ROOT_STATE_STEP_MISMATCH")
    gate = state.get("execution_gate") or {}
    if state.get("next_allowed_step") != "RUN_NEW_ARTICLE_BATCH_NO_STOP":
        raise BoundStartBlocked("BOUND_START_STEP_NOT_107007")
    if gate.get("step_id") != "RUN_NEW_ARTICLE_BATCH_NO_STOP" or int(gate.get("sequence", -1)) != 107007:
        raise BoundStartBlocked("BOUND_START_GATE_NOT_107007")
    if state.get("publish_allowed") is not False:
        raise BoundStartBlocked("BOUND_START_STATE_PUBLISH_NOT_FALSE")

    blocker = state.get("current_execution_blocker") or {}
    if blocker.get("worker_start_allowed") is not True:
        raise BoundStartBlocked("BOUND_START_CURRENT_STATE_START_NOT_ALLOWED")
    ...
    if isinstance(external, dict) and external:
        if external.get("resolved") is not True:
            raise BoundStartBlocked("BOUND_START_EXTERNAL_BLOCKER_NOT_RESOLVED")

    if receipt.get("contract") != RECEIPT_CONTRACT or receipt.get("status") != "PASS":
        raise BoundStartBlocked("BOUND_START_RECEIPT_NOT_PASS")
    if receipt.get("authorized_head_sha") != head:
        raise BoundStartBlocked("BOUND_START_RECEIPT_HEAD_STALE")
    if receipt.get("dispatcher_head_sha") != head:
        raise BoundStartBlocked("BOUND_START_DISPATCHER_HEAD_DRIFT")
    if receipt.get("hardlock_base_status") != "PASS" or receipt.get("hardlock_base_head_sha") != head:
        raise BoundStartBlocked("BOUND_START_HARDLOCK_BASE_NOT_FRESH_PASS")
    if receipt.get("user_approval") is not True:
        raise BoundStartBlocked("BOUND_START_USER_APPROVAL_MISSING")
    if receipt.get("worker_capacity_status") != "AVAILABLE":
        raise BoundStartBlocked("BOUND_START_WORKER_CAPACITY_NOT_AVAILABLE")
    if int(receipt.get("approved_article_count") or 0) < 1:
        raise BoundStartBlocked("BOUND_START_ARTICLE_COUNT_NOT_APPROVED")
    if receipt.get("new_article_policy") != "COMPLETELY_NEW_NO_RECOVERY_BODY":
        raise BoundStartBlocked("BOUND_START_RECEIPT_NEW_ARTICLE_POLICY_INVALID")
    if receipt.get("sequential_advance_policy") != "PASS_ONLY":
        raise BoundStartBlocked("BOUND_START_RECEIPT_ADVANCE_POLICY_INVALID")
    if receipt.get("repair_policy") != "SAME_ARTICLE_SAME_WORKSPACE_UNTIL_PASS":
        raise BoundStartBlocked("BOUND_START_RECEIPT_REPAIR_POLICY_INVALID")
    if receipt.get("restart_recovery_status") != "PASS_CROSS_PROCESS_AND_DURABLE_TRANSPORT_READY":
        raise BoundStartBlocked("BOUND_START_RESTART_RECOVERY_NOT_READY")
    if receipt.get("durable_evidence_transport_status") != "PASS":
        raise BoundStartBlocked("BOUND_START_DURABLE_EVIDENCE_TRANSPORT_NOT_PASS")
    if receipt.get("worker_side_github_write_required") is not False:
        raise BoundStartBlocked("BOUND_START_RECEIPT_WORKER_GITHUB_POLICY_INVALID")
    if receipt.get("publish_allowed") is not False:
        raise BoundStartBlocked("BOUND_START_RECEIPT_PUBLISH_NOT_FALSE")
