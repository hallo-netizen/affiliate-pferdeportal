...
def build(release_receipt_ref: str, final_package_ref: str) -> dict:
    ...
    for row in rows:
        ...
        if ARTICLE_RE.fullmatch(p.name):
            ...
            articles.append({"name": p.name, "plan_slot": slot, "sha256": actual, "byte_length": len(raw), "content_utf8": raw.decode("utf-8")})
    ...
    source_manifest = {
        ...
        "items": [{"ref": f"control/startmaster0107/recovery_sources/{batch}/{a['name']}", "sha256": a["sha256"], "plan_slot": a["plan_slot"]} for a in articles],
    }
    payload = {
    ...
    return {"ok": True, "status": "DELIVERY_HANDOFF_READY", "contract": "PFERDE_ATELIER_CHAT_DELIVERY_ENVELOPE_V1", "batch_sha256": batch, "payload_sha256": sha256_bytes(raw), "gzip_sha256": sha256_bytes(packed), "encoding": "gzip+base64", "payload_b64": encoded, "article_count": 7, "publish_allowed": False, "content_mutation_performed": False}

def selftest() -> dict:
    return {"ok": True, "status": "CHAT_DELIVERY_PAYLOAD_SELFTEST_PASS", "publish_allowed": False, "content_mutation_performed": False}
