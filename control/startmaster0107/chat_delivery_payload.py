...
def validate_import_envelope(pkg: dict, batch: str, articles: list[dict]) -> None:
    ...
    for field, obj in (
        ("fact_pack_bundle_sha256", pkg.get("fact_pack_bundle")),
        ("production_plan_sha256", pkg.get("production_plan")),
        ("workflow_release_sha256", pkg.get("workflow_release")),
    ):
        if not isinstance(obj, dict) or pkg.get(field) != stable_hash(obj):
            raise Blocked("FINAL_PACKAGE_COMPONENT_HASH_INVALID:" + field)
    package_id = stable_hash({
    ...
    plan_by_id = {
        ...
        for row in (pkg["production_plan"].get("items") or []) if isinstance(row, dict)
    }
    if len(release_by_slot) != len(articles):
        raise Blocked("FINAL_PACKAGE_SLOT_COUNT_INVALID")
    for article in articles:
        ...
        if not isinstance(body, str):
            raise Blocked("FINAL_PACKAGE_ARTICLE_BODY_MISSING:" + article["plan_slot"])
        raw = body.encode("utf-8")
        if sha256_bytes(raw) != article["sha256"] or len(raw) != article["byte_length"]:
            raise Blocked("FINAL_PACKAGE_ARTICLE_BYTES_MISMATCH:" + article["plan_slot"])
...
def build(release_receipt_ref: str, final_package_ref: str) -> dict:
    ...
    if not package_path.is_file() or package_path.name != FINAL_NAME:
        raise Blocked("FINAL_PACKAGE_MISSING_OR_NAME_INVALID")
    ...
    payload = {
        ...
        "final_package_sha256": sha256_file(package_path),
        "import_envelope_json": import_raw.decode("utf-8"),
        "import_envelope_sha256": import_sha,
