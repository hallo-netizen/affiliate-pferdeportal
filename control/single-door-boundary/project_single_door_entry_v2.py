...
def _pointer_authority(repo: Path) -> None:
    ...
    manifest_sha = str(obj.get("h8_boundary_sha256") or "")
    ...
    if not mp.is_file() or _sha(mp) != manifest_sha:
    ...
    for row in bindings:
        ...
        seen.add(ref)
        fp = repo / ref
        if not fp.is_file() or _sha(fp) != str(row.get("sha256") or ""):
...
def current_runtime(repo: Path) -> Mapping[str, Any]:
    guard = _module(
        Path(repo) / "control/startmaster0107/runtime_inbox/runtime_batch_slot_guard.py",
    ...
    *,
    ...
    if status == "READY_IDLE":
        return {
            ...
            "status": "PROJECT_ARMED_NO_ACTIVE_BATCH",
            "worker_request": None,
    ...
    if status == "READY_WAITING_PACKAGE":
        ...
        if incoming_provenance_provider is None and not incoming.exists():
            ...
            return {
                "ok": True,
                "contract": PROJECT_ENTRY_CONTRACT,
                "status": "PREPRODUCTION_BOOTSTRAP_SINGLE_DOOR_REQUIRED",
                "room_token": boot.ROOM_TOKEN,
                "worker_request": req,
                "next_room_after_pass": boot.NEXT_ROOM_TOKEN,
                "authoritative_execution_origin": "SINGLE_DOOR_EXECUTOR_ONLY",
                "publish_allowed": False,
            }
        try:
        ...
        return {
            ...
            "room_token": pre.ROOM_TOKEN,
            "worker_request": req,
            "next_room_after_pass": pre.NEXT_ROOM_TOKEN,
...
def main(argv: list[str]) -> int:
    try:
        if argv not in ([], ["status"]):
            raise ProjectEntryBlocked("USAGE_STATUS_ONLY")
        print(json.dumps(resolve_entry(repo=REPO_ROOT), ensure_ascii=False, indent=2))
        return 0
    except Exception as exc:
        print(json.dumps({
            "ok": False,
            "status": "PROJECT_SINGLE_DOOR_ENTRY_BLOCKED",
            "error": str(exc),
            "publish_allowed": False,
