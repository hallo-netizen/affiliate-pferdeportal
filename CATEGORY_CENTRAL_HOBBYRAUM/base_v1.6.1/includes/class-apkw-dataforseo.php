...
final class APKW_DataForSEO {
    ...
    private static function request(string $method, string $endpoint, ?string $body): array {
        ...
        if ($http_code < 200 || $http_code >= 300) { throw new RuntimeException('DataForSEO HTTP-Fehler ' . $http_code . '.'); }
        try { $decoded = json_decode($raw_body, true, 512, JSON_THROW_ON_ERROR); }
        catch (Throwable $e) { throw new RuntimeException('DataForSEO-Antwort ist kein gültiges JSON.'); }
        if (!is_array($decoded)) { throw new RuntimeException('DataForSEO-Antwortformat ungültig.'); }
