...
final class Pferdeportal_Affiliate_Router {
    ...
    const VERSION = '6.4.0';
    const EBAY_RUNTIME_BUILD = '6.4.0-ebay-atomic-last-known-good-recovery-20260809';
    ...
    const EBAY_SCHEMA_VERSION = '2.0';
    const EBAY_CONTENT_POLICY_VERSION = '4.0';
    const EBAY_PRIVATE_CLASSIFIER_VERSION = '3.0';
    const EBAY_BUSINESS_CLASSIFIER_VERSION = '3.0';
    ...
    private function __construct() {
        ...
        add_action('init', array($this, 'ensure_ebay_schedule'), 22);
        add_action('init', array($this, 'ensure_ebay_refresh_schedule'), 23);
        add_action('init', array($this, 'ensure_ebay_maintenance_schedule'), 24);
        // V5.22: one-time non-destructive flattening of the private eBay taxonomy.
        // Keeps existing child term IDs and listing assignments, removes only the obsolete provider layer.
        add_action('init', array($this, 'maybe_migrate_ebay_private_flat_structure'), 25);
        add_action(self::EBAY_CRON_HOOK, array($this, 'run_ebay_sync'));
        add_action(self::EBAY_WORKER_HOOK, array($this, 'run_ebay_sync_worker'));
        add_action(self::EBAY_REFRESH_CRON_HOOK, array($this, 'run_ebay_inventory_refresh'));
        add_action(self::EBAY_REFRESH_WORKER_HOOK, array($this, 'run_ebay_inventory_refresh_worker'));
        add_action(self::EBAY_MAINTENANCE_CRON_HOOK, array($this, 'run_ebay_maintenance_v2'));
        ...
        if (is_admin()) {
            ...
            add_action('admin_post_ppar_ebay_worker_now', array($this, 'handle_ebay_worker_now'));
            add_action('wp_ajax_ppar_ebay_admin_pump', array($this, 'handle_ebay_admin_pump'));
            add_action('admin_post_ppar_ebay_worker_tick', array($this, 'handle_ebay_worker_tick'));
            add_action('admin_post_nopriv_ppar_ebay_worker_tick', array($this, 'handle_ebay_worker_tick'));
            add_action('admin_post_ppar_ebay_run_refresh', array($this, 'handle_ebay_run_refresh'));
            add_action('admin_post_ppar_ebay_refresh_worker_tick', array($this, 'handle_ebay_refresh_worker_tick'));
            add_action('admin_post_nopriv_ppar_ebay_refresh_worker_tick', array($this, 'handle_ebay_refresh_worker_tick'));
    ...
    private function render_article_product_block($post_id, $forced_campaign_post_ids = array(), $preview_count = 0) {
        ...
        foreach ($campaigns as $campaign) {
            ...
            $html = $this->render_banner($banner, $post_id, $context, $group, 'post_bottom_products');
            if ($html !== '') {
    ...
    private function fixed_campaign_selection($post_id, $slot_type, $reason) {
        ...
        if (!$campaign || empty($campaign['active']) || !$this->campaign_is_complete($campaign) || !$this->rule_is_current($campaign) || !$this->campaign_program_allows_delivery($campaign) || !$this->campaign_control_allows_delivery($campaign, $slot_type) || !$this->campaign_health_allows_delivery($campaign)) { return null; }
            ...
            if ($selection && !empty($selection['campaign'])) {
                ...
                if ($group && $banner) {
                    ...
                    if (trim($html) !== '') {
                        ...
                        $out = '<div class="' . esc_attr(implode(' ', $classes)) . '" data-ppar-slot="' . esc_attr($slot_type) . '" data-ppar-group="' . esc_attr($group['id'] ?? '') . '">';
                        $out .= $disclosure;
    ...
    private function render_banner($banner, $post_id, $context, $group, $slot_type) {
        ...
        if ($title !== '' || $description !== '' || $button !== '') {
            $out .= '<span class="ppar-banner-text">';
            if ($title !== '') {
                $out .= '<strong class="ppar-banner-title">' . esc_html($title) . '</strong>';
            }
            if ($description !== '') {
                $out .= '<span class="ppar-banner-description">' . esc_html($description) . '</span>';
            }
            if ($creative_type === 'product' && ($price !== '' || $availability !== '')) {
                $out .= '<span class="ppar-banner-meta">';
                if ($price !== '') {
                    $out .= '<strong class="ppar-banner-price">' . esc_html($price . ($currency !== '' ? ' ' . $currency : '')) . '</strong>';
                }
                if ($availability !== '') {
                    $out .= '<span class="ppar-banner-availability">' . esc_html($availability) . '</span>';
                }
                $out .= '</span>';
            }
            if ($button !== '') {
    ...
    private function ranked_campaigns_for_slot($context, $slot_type, $forced_campaign_id = '') {
        $candidates = array();
        foreach ($this->get_campaigns() as $campaign) {
            if (!is_array($campaign) || empty($campaign['active']) || !$this->campaign_is_complete($campaign) || !$this->rule_is_current($campaign) || !$this->campaign_program_allows_delivery($campaign) || !$this->campaign_control_allows_delivery($campaign, $slot_type ?? '') || !$this->campaign_health_allows_delivery($campaign)) {
                continue;
            }
    ...
    public function admin_assets() {
        ...
        if (!in_array($page, array('affiliate-portal-zentrale', 'affiliate-portal-creatives', 'affiliate-portal-campaign-edit', 'affiliate-portal-assignments', 'affiliate-portal-preview'), true)) {
            return;
        }
...
JS;
    ...
    private function ebay_business_diag_campaign_map() {
        $map = array();
    ...
    private function ebay_business_diag_block_reason($row, $object, $campaign) {
        ...
        if (method_exists($this, 'ebay_business_campaign_source_allows_delivery') && !$this->ebay_business_campaign_source_allows_delivery($campaign)) {
            return 'Kampagne ist gespeichert, aber die eBay-Quelle ist nicht frisch/freigegeben; öffentliche Ausgabe bleibt fail-closed.';
        }
        return '';
    }
    ...
    private function ebay_business_diag_resolve_public_target($target_key) {
        ...
        if ($kind === 'journal') {
            $post = get_page_by_path($slug, OBJECT, 'page');
            if (!$post && $slug === 'journal') { $post = get_page_by_path('journal', OBJECT, 'page'); }
            if (!$post) { return null; }
            $page_id = absint($post->ID ?? 0);
            if ($page_id <= 0) { return null; }
    ...
    private function ebay_business_live_slot_references($campaigns, $limit = 20) {
        ...
        foreach ((array) $campaigns as $campaign) {
            if (!is_array($campaign)
                ...
                || absint(get_post_meta(absint($campaign['post_id'] ?? 0), '_ppar_ebay_business_auto', true)) !== 1
                || (method_exists($this, 'ebay_business_campaign_source_allows_delivery') && !$this->ebay_business_campaign_source_allows_delivery($campaign))) {
                continue;
            }
            foreach ((array) ($campaign['automation_target_keys'] ?? array()) as $target_key) {
                $target = $this->ebay_business_diag_resolve_public_target($target_key);
                if (!is_array($target)) { continue; }
                foreach ((array) ($target['slots'] ?? array()) as $slot) {
                    $slot = sanitize_key((string) $slot);
                    if (!$this->campaign_slot_allowed($campaign, $slot)) { continue; }
                    $selected = $this->select_campaign_for_slot((array) $target['context'], $slot);
                    if (!$selected || empty($selected['campaign'])) { continue; }
                    $selected_campaign = $selected['campaign'];
                    if (absint($selected_campaign['post_id'] ?? 0) !== absint($campaign['post_id'] ?? 0)) { continue; }
                    $key = sanitize_key((string) $target_key) . '|' . $slot;
                    if (isset($seen[$key])) { continue; }
                    $seen[$key] = true;
                    $refs[] = array(
                        'page_label'=>sanitize_text_field((string) ($target['label'] ?? '')),
                        'url'=>esc_url_raw((string) ($target['url'] ?? '')),
                        'slot'=>$slot,
                        'product'=>sanitize_text_field((string) ($campaign['title'] ?? $campaign['name'] ?? '')),
                        'campaign_id'=>absint($campaign['post_id'] ?? 0),
                        'reason'=>sanitize_text_field((string) ($selected['reason'] ?? '')),
                    );
                    if (count($refs) >= $limit) { return $refs; }
                }
            }
        }
        return $refs;
    ...
    public function render_ebay_business_assignments_page() {
        ...
        $campaign_map = $this->ebay_business_diag_campaign_map();
        $live_refs = $this->ebay_business_live_slot_references($campaign_map, 20);
        $pending_assets = method_exists($this, 'ebay_business_pending_asset_count') ? $this->ebay_business_pending_asset_count() : 0;

        $kpi = array('all'=>$total,'available'=>0,'blocked_content'=>0,'verified'=>0,'deliverable'=>0,'attention'=>0);
        ...
        foreach ((array) $campaign_map as $campaign) {
            if (!is_array($campaign)
                || empty($campaign['active'])
                || sanitize_key((string) ($campaign['network'] ?? '')) !== 'ebay'
                || sanitize_key((string) ($campaign['creative_type'] ?? '')) !== 'product'
                || absint(get_post_meta(absint($campaign['post_id'] ?? 0), '_ppar_ebay_business_auto', true)) !== 1) { continue; }
            if (!$this->ebay_business_campaign_source_allows_delivery($campaign)) { continue; }
            $kpi['deliverable']++;
        }
        $kpi['attention'] = max(0, $kpi['available'] - $kpi['deliverable']);
        ...
        <div class="wrap ppar-ebay-business-diag">
            ...
            <div class="ppar-ebay-business-kpis">
                ...
                <div class="ppar-ebay-business-kpi"><span>Kampagnen lieferbar</span><strong><?php echo absint($kpi['deliverable']); ?></strong></div>
                <div class="ppar-ebay-business-kpi"><span>Quelle aktiv, nicht lieferbar</span><strong><?php echo absint($kpi['attention']); ?></strong></div>
            </div>
            ...
            <div class="ppar-ebay-business-box">
                <h2>Konkrete Frontend-Referenzen</h2>
                <?php if (!$live_refs) : ?>
                    <p><strong>Noch keine Router-Referenz.</strong> Damit ist belegt, dass aktuell kein eBay-BUSINESS-Produkt einen realen Produktslot als Gewinner erreicht. Die Tabelle darunter zeigt den Blockiergrund.</p>
                <?php else : ?>
                    <p>Diese Zeilen sind keine Vermutung: Die Affiliate-Zentrale wählt das genannte eBay-Produkt für den genannten Slot aktuell als Gewinner aus.</p>
                    <table class="widefat striped"><thead><tr><th>Seite/Kategorie</th><th>Slot</th><th>Produkt</th><th>Auswahlgrund</th></tr></thead><tbody>
                    <?php foreach ($live_refs as $ref) : ?><tr>
                        ...
                        <td><?php echo esc_html($ref['reason']); ?></td>
                    </tr><?php endforeach; ?>
                    </tbody></table>
                <?php endif; ?>
            </div>

            <form method="get" style="margin:18px 0;display:flex;gap:8px;align-items:center"><input type="hidden" name="page" value="affiliate-portal-ebay-business"><input type="search" name="s" value="<?php echo esc_attr($search); ?>" class="regular-text" placeholder="Produkt, Item-ID oder Händler"><?php submit_button('Filtern','secondary','',false); ?></form>
            ...
            <?php if (!$rows) : ?><tr><td colspan="7">Keine eBay-BUSINESS-Produkte gefunden.</td></tr><?php else : foreach ($rows as $row) :
                ...
                $published = is_array($object)
                    ...
                    && is_array($campaign)
                    && !empty($campaign['active'])
                    && $this->ebay_business_campaign_source_allows_delivery($campaign);
            ...
            ?><tr>
                ...
                <td><?php if ($published) : ?><span class="ppar-ebay-business-ok">ausspielbar</span><?php elseif (is_array($object)) : ?><span class="ppar-ebay-business-warn"><?php echo esc_html((string) ($object['status'] ?? 'review')); ?></span><?php else : ?><span class="ppar-ebay-business-warn">nicht geplant</span><?php endif; ?></td>
                <td><?php echo $reason === '' ? '<span class="ppar-ebay-business-ok">Kein Blockiergrund im Ausgabeobjekt.</span>' : esc_html($reason); ?></td>
