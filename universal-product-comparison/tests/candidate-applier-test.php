...
class FakeComparisonMaintenance { public $scope_error=null; public $seen=[]; function reevaluation_scope_for_new_product($id){$this->seen[]=$id; return $this->scope_error ?: ['product_id'=>$id,'product_group_key'=>'regendecken','peer_product_ids'=>[5,12],'existing_comparison_ids'=>[3,9],'automatic_comparison_created'=>false];} }
...
$db=new FakeDB();$k=new FakeKnowledge();$m=new FakeMaintenance();$cm=new FakeComparisonMaintenance();$a=new UPC_Candidate_Applier($db,$k,$m,$cm);
$x=$a->apply($c,$r);ok($x['status']==='NEW_PRODUCT_ACCEPTED'&&$x['reevaluate_product_group']===true&&$x['automatic_comparison_created']===false&&$x['article_write']===false,'new candidate accepted without comparison/article');ok($x['reevaluation_scope']['peer_product_ids']===[5,12]&&$x['reevaluation_scope']['existing_comparison_ids']===[3,9],'accepted candidate returns bounded same-group reevaluation scope');ok($cm->seen===[77],'reevaluation scope calculated for created product');ok($db->queries===['START TRANSACTION','COMMIT'],'successful apply commits atomically');ok($k->created[0]['lifecycle_status']==='UNKNOWN','lifecycle is not guessed');
...
$db=new FakeDB();$k=new FakeKnowledge();$k->create_error=new WP_Error('CREATE_FAIL');$a=new UPC_Candidate_Applier($db,$k,$m,$cm);$x=$a->apply($c,$r);ok(is_wp_error($x)&&$x->code==='CREATE_FAIL'&&end($db->queries)==='ROLLBACK','create failure rolls back');
$db=new FakeDB();$k=new FakeKnowledge();$k->fact_error_at=1;$a=new UPC_Candidate_Applier($db,$k,$m,$cm);$x=$a->apply($c,$r);ok(is_wp_error($x)&&$x->code==='FACT_FAIL'&&end($db->queries)==='ROLLBACK','fact failure rolls back');
$db=new FakeDB();$k=new FakeKnowledge();$cm_fail=new FakeComparisonMaintenance();$cm_fail->scope_error=new WP_Error('SCOPE_FAIL');$a=new UPC_Candidate_Applier($db,$k,$m,$cm_fail);$x=$a->apply($c,$r);ok(is_wp_error($x)&&$x->code==='SCOPE_FAIL'&&end($db->queries)==='ROLLBACK','reevaluation scope failure rolls back candidate acceptance');
...
$a=new UPC_Candidate_Applier(new FakeDB(),new FakeKnowledge(),new FakeMaintenance(),null);$x=$a->apply($c,$r);ok(is_wp_error($x)&&$x->code==='UPC_CANDIDATE_REEVALUATION_RUNTIME_MISSING','missing reevaluation runtime blocks before write');
